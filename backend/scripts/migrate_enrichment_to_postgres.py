"""Copy enriched song data from local SQLite to remote Postgres (Supabase)."""
...
def main() -> None:
    ...
    try:
        ...
        print(f"Found {len(ids)} enriched songs in SQLite", flush=True)

        updated = 0
        missing = 0
        for start in range(0, len(ids), BATCH_SIZE):
            batch_ids = ids[start : start + BATCH_SIZE]
            src_rows = sqlite_db.query(Song).filter(Song.id.in_(batch_ids)).all()
            video_ids = [s.yt_video_id for s in src_rows]
            dst_by_video = {
                s.yt_video_id: s
                for s in pg_db.query(Song)
                .filter(Song.yt_video_id.in_(video_ids), Song.deleted_at.is_(None))
                .all()
            }
            for src in src_rows:
                ...
                updated += 1
            pg_db.commit()
            print(f"  ... {min(start + BATCH_SIZE, len(ids))}/{len(ids)} processed", flush=True)

        print(f"Done: {updated} updated, {missing} not found in Postgres", flush=True)
    finally:
        sqlite_db.close()
        pg_db.close()
